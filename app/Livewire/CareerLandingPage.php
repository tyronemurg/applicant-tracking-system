...
class CareerLandingPage extends Component
{
    ...
    {
        return JobOpenings::query()->jobStillOpen()->where('published_career_site', '=', 1)->orderBy('created_at', 'desc');
    }
    ...
    private function jobTypes(): void
    {
        ...
        $this->jobTypeList = static::queryTable()
            ->select(['JobType', 'created_at']) // Include created_at in the select list
            ->distinct()
            ->get()
            ->toArray();
    }
